...
type Props = { brand?: string; descriptor?: string; navItems?: NavItem[]; ctaHref?: string; ctaLabel?: string };

export function Header({ brand = "Nexo Local Studio", descriptor = "Landing pages premium", navItems = [{ label: "Servicios", href: "#servicios" }, { label: "Paquetes", href: "#paquetes" }, { label: "Demos", href: "/demos" }, { label: "FAQ", href: "#faq" }], ctaHref = "#contacto", ctaLabel = "Solicitar diagnóstico" }: Props) {
  return (
    <header className="sticky top-0 z-40 border-b border-brand-border/80 bg-brand-background/90 backdrop-blur-xl">
      <div className="container-page flex h-20 items-center justify-between gap-5">
        <Link href="/" className="group flex items-center gap-3" aria-label="Ir al inicio">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-brand-primary text-sm font-bold text-white shadow-card transition group-hover:-translate-y-0.5">NL</span>
