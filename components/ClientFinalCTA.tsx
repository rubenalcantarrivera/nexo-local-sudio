  ...
  return (
    <section className="container-page pb-16 sm:pb-20 lg:pb-28">
      <div className="relative overflow-hidden rounded-[2.5rem] bg-brand-primary text-white shadow-soft">
        {image ? <img src={image.src} alt={image.alt} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover opacity-35" /> : null}
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(11,18,32,.92),rgba(11,18,32,.64),rgba(11,18,32,.28))]" />
        <div className="relative max-w-3xl p-7 sm:p-10 lg:p-14">
          <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-white/60">{config.location}</p>
          ...
          {config.disclaimer ? <p className="mt-5 max-w-2xl border-l border-white/30 pl-4 text-sm text-white/60">{config.disclaimer}</p> : null}
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <ButtonLink href={whatsappHref} className="!bg-white !text-brand-primary">{config.hero.primaryCta}</ButtonLink>
            <ButtonLink href="#ubicacion" variant="secondary" className="!border-white/25 !bg-white/10 !text-white hover:!bg-white/16">Ver ubicación</ButtonLink>
          </div>
        </div>
      </div>
    </section>
