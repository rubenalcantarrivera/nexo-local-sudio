    ...
    <footer className={isClient ? "border-t border-brand-border bg-white" : "dark-grain texture-overlay relative overflow-hidden text-white"}>
      <div className="container-page grid gap-10 py-14 md:grid-cols-[1.25fr_0.75fr_0.9fr]">
        <div>
          ...
          <p className={isClient ? "mt-5 max-w-md text-sm text-brand-muted" : "mt-7 max-w-md text-sm text-white/78"}>{isClient ? "Servicios, ubicación y contacto directo para facilitar tu próxima visita." : "Estudio digital boutique para negocios locales. Creamos páginas web claras, profesionales y conectadas a WhatsApp."}</p>
        </div>
        <div><p className={isClient ? "text-sm font-semibold text-brand-primary" : "text-sm font-semibold text-white"}>Explorar</p><div className={isClient ? "mt-4 flex flex-col gap-3 text-sm text-brand-muted" : "mt-4 flex flex-col gap-3 text-sm text-white/78"}>{isClient ? <><Link href="#servicios" className="hover:text-brand-primary">Servicios</Link><Link href="#ubicacion" className="hover:text-brand-primary">Ubicación</Link><Link href="#faq" className="hover:text-brand-primary">Preguntas Frecuentes</Link></> : <><Link href="/demos" className="hover:text-white">Ejemplos por nicho</Link><Link href="/#paquetes" className="hover:text-white">Precios</Link><Link href="/#proceso" className="hover:text-white">Proceso</Link><Link href="/#faq" className="hover:text-white">Preguntas Frecuentes</Link></>}</div></div>
        <div><p className={isClient ? "text-sm font-semibold text-brand-primary" : "text-sm font-semibold text-white"}>Contacto</p><div className={isClient ? "mt-4 flex flex-col gap-3 text-sm text-brand-muted" : "mt-4 flex flex-col gap-3 text-sm text-white/78"}><span>{displayLocation}</span>{isClient ? <span>WhatsApp disponible para citas e información.</span> : <><a href={`mailto:${agency.email}`} className="hover:text-white">{agency.email}</a><span>WhatsApp: {agency.phoneDisplay}</span></>}</div></div>
      </div>
      <div className={isClient ? "border-t border-brand-border py-5" : "border-t border-white/10 py-5"}><div className={isClient ? "container-page flex flex-col gap-2 text-xs text-brand-muted sm:flex-row sm:items-center sm:justify-between" : "container-page flex flex-col gap-2 text-xs text-white/76 sm:flex-row sm:items-center sm:justify-between"}><span>© {new Date().getFullYear()} {displayName}.</span><span>{isClient ? "Atención sujeta a disponibilidad y valoración cuando aplique." : "Estudio boutique con estructura ligera y comunicación directa."}</span></div></div>
    </footer>
  );
}
